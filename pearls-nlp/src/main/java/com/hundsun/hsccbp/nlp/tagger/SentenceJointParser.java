...
public class SentenceJointParser {
	 ...
	 * @return
	 */
	protected List<List<String>> jointParse(String sentence) {
			String[][] s = cnFactory.getPos().tag2Array(sentence);
			DependencyTree tree = cnFactory.getParser()
					.parse2T(s[0], s[1]);
	 ...
	 * @param sentence
	 */
	private void coreferenceResolution(List<List<String>> dependencyList,String sentence){
		LinkedList<EntityGroup> ll = cnFactory.getRuleAnaphora()
				.resolve(sentence);
